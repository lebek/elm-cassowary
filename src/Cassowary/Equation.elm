...
-- by convention always lte, ie e1 <= e2
...
extendExpression expr ast =
    case ( expr, ast ) of
        ( { vars, const }, Var n ) ->
            Expression
                (Maybe.withDefault (Dict.insert n 1 vars)
        ...
        ( { vars, const }, Lit n ) ->
            Expression vars (const + n)

        ( lexpr, Coeff _ 0 ) ->
            lexpr

        ( { vars, const }, Coeff (Var n) x ) ->
            Expression
                (Maybe.withDefault (Dict.insert n x vars)
        ...
        ( lexpr, Add e1 e2 ) ->
            extendExpression (extendExpression lexpr e2) e1

        ( _, _ ) ->
            Debug.crash "Unhandled expression"


makeExpression : ExpressionAst -> Expression
makeExpression =
    extendExpression { vars = Dict.empty, const = 0 }
